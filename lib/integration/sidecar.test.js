...
import { createSidecarRouter } from '../forge-service.js';
...
// ── Auth Tier tests via createSidecarRouter ──────────────────────────────────

function makeRouterCtx(configOverrides = {}, envOverrides = {}) {
  const config = {
    auth: { mode: 'trust', claimsPath: 'sub', adminToken: null, metricsToken: null },
    defaultModel: 'claude-sonnet-4-6',
    defaultHitlLevel: 'cautious',
    adminKey: 'admin-secret',
    conversation: { window: 25 },
...
async function startRouterServer(ctx) {
  const handler = createSidecarRouter(ctx);
  const srv = createServer(handler);
  return new Promise(resolve => {
    srv.listen(0, '127.0.0.1', () => {
      const { port } = srv.address();
      resolve({ server: srv, port, baseUrl: `http://127.0.0.1:${port}` });
    });
  });
}

describe('Auth Tiers — createSidecarRouter', () => {
  let server;
  ...
  it('/health is always 200 regardless of auth', async () => {
    const ctx = makeRouterCtx();
    const srv = await startRouterServer(ctx);
    server = srv.server;
    const res = await fetch(`${srv.baseUrl}/health`);
    expect(res.status).toBe(200);
  });

  it('Admin route without adminToken or adminKey → 503', async () => {
    const ctx = makeRouterCtx({ adminKey: null, auth: { mode: 'trust', claimsPath: 'sub', adminToken: null } });
    const srv = await startRouterServer(ctx);
    server = srv.server;
    const res = await fetch(`${srv.baseUrl}/forge-admin/config`);
    expect(res.status).toBe(503);
  });

  it('Admin route with wrong token → 401 + WWW-Authenticate header', async () => {
    const ctx = makeRouterCtx();
    const srv = await startRouterServer(ctx);
    server = srv.server;
    const res = await fetch(`${srv.baseUrl}/forge-admin/config`, {
  ...
  it('Admin route with correct adminKey → 200 (legacy compat)', async () => {
    const ctx = makeRouterCtx();
    const srv = await startRouterServer(ctx);
    server = srv.server;
    const res = await fetch(`${srv.baseUrl}/forge-admin/config`, {
  ...
  it('Admin route with correct adminToken → 200 (new field)', async () => {
    const ctx = makeRouterCtx({
      adminKey: null,
      auth: { mode: 'trust', claimsPath: 'sub', adminToken: 'new-admin-token' }
    });
    const srv = await startRouterServer(ctx);
    server = srv.server;
    const res = await fetch(`${srv.baseUrl}/forge-admin/config`, {
  ...
  it('/agent-api/evals/summary without auth → 401 (security regression guard)', async () => {
    const ctx = makeRouterCtx();
    const srv = await startRouterServer(ctx);
    server = srv.server;
    const res = await fetch(`${srv.baseUrl}/agent-api/evals/summary`);
    expect(res.status).toBe(401);
  });

  it('/agent-api/evals/summary with correct adminKey → 200', async () => {
    const ctx = makeRouterCtx();
    const srv = await startRouterServer(ctx);
    server = srv.server;
    const res = await fetch(`${srv.baseUrl}/agent-api/evals/summary`, {
  ...
  it('auth.mode: none → all routes open including admin and eval', async () => {
    const ctx = makeRouterCtx({
      adminKey: null,
      auth: { mode: 'none', claimsPath: 'sub', adminToken: null }
    });
    const srv = await startRouterServer(ctx);
    server = srv.server;
    const [evalRes, adminRes, healthRes] = await Promise.all([
      fetch(`${srv.baseUrl}/agent-api/evals/summary`),
      fetch(`${srv.baseUrl}/forge-admin/config`),
  ...
  it('/agent-api/evals/runs without auth → 401', async () => {
    const ctx = makeRouterCtx();
    const srv = await startRouterServer(ctx);
    server = srv.server;
    const res = await fetch(`${srv.baseUrl}/agent-api/evals/runs`);
    expect(res.status).toBe(401);
  });

  it('/agent-api/evals/runs with correct adminKey → 200', async () => {
    const ctx = makeRouterCtx();
    const srv = await startRouterServer(ctx);
    server = srv.server;
    const res = await fetch(`${srv.baseUrl}/agent-api/evals/runs`, {
  ...
  it('/agent-api/v1/evals/summary is normalized and gated as admin tier', async () => {
    const ctx = makeRouterCtx();
    const srv = await startRouterServer(ctx);
    server = srv.server;
    // Without auth → 401 (proves normalization routes it to tier 2)
    const unauthed = await fetch(`${srv.baseUrl}/agent-api/v1/evals/summary`);
    expect(unauthed.status).toBe(401);
    // With correct token → 200
    const authed = await fetch(`${srv.baseUrl}/agent-api/v1/evals/summary`, {
      headers: { Authorization: 'Bearer admin-secret' }
    });
    expect(authed.status).toBe(200);
  });

  it('Tier 1 route without JWT → 401 + WWW-Authenticate header', async () => {
    const ctx = makeRouterCtx();
    const srv = await startRouterServer(ctx);
    server = srv.server;
    const res = await fetch(`${srv.baseUrl}/agent-api/chat`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ message: 'hi' })
    });
  ...
  it('/metrics is open (404, not 401) when metricsToken not configured', async () => {
    const ctx = makeRouterCtx();
    const srv = await startRouterServer(ctx);
    server = srv.server;
    const res = await fetch(`${srv.baseUrl}/metrics`);
    // Auth gate passes (tier 3, no metricsToken), route falls to 404 (no handler yet — v0.5.0)
    expect(res.status).toBe(404);
    expect(res.headers.get('www-authenticate')).toBeNull();
  });

  it('/metrics → 401 when metricsToken set and wrong token provided', async () => {
    const ctx = makeRouterCtx({ auth: { mode: 'trust', claimsPath: 'sub', adminToken: null, metricsToken: 'scrape-secret' } });
    const srv = await startRouterServer(ctx);
    server = srv.server;
    const res = await fetch(`${srv.baseUrl}/metrics`, {
      headers: { Authorization: 'Bearer wrong-token' }
  ...
  it('/metrics → pass auth (404) when metricsToken set and correct token provided', async () => {
    const ctx = makeRouterCtx({ auth: { mode: 'trust', claimsPath: 'sub', adminToken: null, metricsToken: 'scrape-secret' } });
    const srv = await startRouterServer(ctx);
    server = srv.server;
    const res = await fetch(`${srv.baseUrl}/metrics`, {
    ...
    expect(res.status).toBe(404); // auth gate passed; no handler yet
  });

  it('adminToken with ${VAR} env reference is resolved', async () => {
    const ctx = makeRouterCtx(
      { adminKey: null, auth: { mode: 'trust', claimsPath: 'sub', adminToken: '${ADMIN_KEY}' } },
      { ADMIN_KEY: 'resolved-key' }
    );
    const srv = await startRouterServer(ctx);
    server = srv.server;
    const res = await fetch(`${srv.baseUrl}/forge-admin/config`, {
