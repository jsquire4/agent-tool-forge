...
import { createAuth, authenticateAdmin, resolveSecret } from './auth.js';
...
describe('auth', () => {
  ...
  describe('resolveSecret', () => {
    it('returns a literal string as-is', () => {
      expect(resolveSecret('my-secret', {})).toBe('my-secret');
    });

    it('resolves ${VAR} from env', () => {
      expect(resolveSecret('${MY_KEY}', { MY_KEY: 'abc123' })).toBe('abc123');
    });

    it('returns null when ${VAR} not in env', () => {
      expect(resolveSecret('${MISSING}', {})).toBeNull();
    });
    ...
    it('defaults env to empty object when omitted', () => {
      expect(resolveSecret('${ANY}')).toBeNull();
    });

    it('handles explicit null env without throwing', () => {
      expect(resolveSecret('${FOO}', null)).toBeNull();
    });
  });
