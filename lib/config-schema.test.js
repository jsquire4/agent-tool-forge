...
describe('config-schema', () => {
  ...
  describe('validateConfig', () => {
    ...
    it('passes when auth.mode is verify with signingKey present', () => {
      const { valid } = validateConfig({ auth: { mode: 'verify', signingKey: 'secret' } });
      expect(valid).toBe(true);
    });

    it('accepts auth.mode: none', () => {
      const { valid } = validateConfig({ auth: { mode: 'none' } });
      expect(valid).toBe(true);
    });
