...
describe('handleAgents', () => {
  ...
  it('PATCH /forge-admin/agents → 405 Method not allowed', async () => {
    const res = makeRes();
    await handleAgents(makeReq('PATCH', '/forge-admin/agents/support'), res, ctx);
    expect(res.statusCode).toBe(405);
  });

  it('DELETE sole default agent — no auto-promote when no other agents remain', async () => {
    ctx.agentRegistry.upsertAgent({ agent_id: 'only', display_name: 'Only Agent' });
    ctx.agentRegistry.setDefault('only');
    const res = makeRes();
    await handleAgents(makeReq('DELETE', '/forge-admin/agents/only'), res, ctx);
    expect(res.statusCode).toBe(200);
    const all = await ctx.agentRegistry.getAllAgents();
