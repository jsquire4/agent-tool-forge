...
describe('forge-service /mcp auth (Group 4)', () => {
  ...
  it('Unknown route → 401 (auth gate fires before route check)', async () => {
    // With sidecarCtx initialized and auth.mode: trust, all app-tier routes require
    // a JWT before dispatch. An unknown route is treated as tier-1 (app), so an
