...
describe('Admin API', () => {
  ...
  it('valid PUT request: updates section', async () => {
    const res = makeRes();
    await handleAdminConfig(
      makeReq('PUT', '/forge-admin/config/model', { defaultModel: 'gpt-4o' }),
      res, makeCtx()
    );
  ...
  it('invalid section: 400', async () => {
    const res = makeRes();
    await handleAdminConfig(
      makeReq('PUT', '/forge-admin/config/invalid', {}),
      res, makeCtx()
    );
  ...
  it('GET returns merged config', async () => {
    ...
    const res = makeRes();
    await handleAdminConfig(
      makeReq('GET', '/forge-admin/config', null),
      res, ctx
    );
    expect(res.writeHead).toHaveBeenCalledWith(200, expect.any(Object));
  ...
  it('PUT model section updates config in ctx', async () => {
    ...
    await handleAdminConfig(
      makeReq('PUT', '/forge-admin/config/model', { defaultModel: 'gpt-4o-mini' }),
      putRes, ctx
    );
  ...
  describe('overlay persistence', () => {
    ...
    it('continues without error when configPath is not provided', async () => {
      ...
      const res = makeRes();
      await handleAdminConfig(
        makeReq('PUT', '/forge-admin/config/hitl', { defaultHitlLevel: 'paranoid' }),
        res, ctx
      );
      expect(res.writeHead).toHaveBeenCalledWith(200, expect.any(Object));
    ...
    it('persists conversation section to forge.config.json', async () => {
      ...
      const res = makeRes();
      await handleAdminConfig(
        makeReq('PUT', '/forge-admin/config/conversation', { window: 50 }),
        res, ctx
      );
  ...
  it('returns 405 for unsupported methods', async () => {
    const res = makeRes();
    await handleAdminConfig(
      makeReq('DELETE', '/forge-admin/config/model', null),
      res, makeCtx()
    );
